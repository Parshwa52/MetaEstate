...
contract morter
{
    ...
    struct House_property 
    {
        ...
        uint mortgageamt;
        uint downpayment;
        uint capital_to_be_raised;
        uint emi_to_be_paid;
        address[] risk_free_investors;
        uint[] riskFreeInvestorPayment;
        uint[] riskFreeInvestorInvestment;
        bool firstEmiDone;
        uint totalInterestMonths;
        uint interestMonthsLeft;
        uint nftTokenId;
        uint lastEMItimestamp;
    }

    uint public propertycounter=0;

    mapping(uint=>House_property) public allproperties;

    function listProperty(uint _price,uint tokenId) public
    ...
    function direct_buy_property(uint property_id,uint payToSeller,uint payToTC)public payable
    {
        ...
        nftContract.transferFromOwnerToMortgager(prop.owner,msg.sender,prop.nftTokenId);
        prop.owner=msg.sender;
        allproperties[property_id]=prop;
    }

    
    function initiate_mortgage(uint property_id, uint mortgage_amt,uint _emi,uint interestMonths)public payable
    {
        ...
        require(msg.value==allproperties[property_id].price-mortgage_amt,"downpayment");
        House_property memory prop = allproperties[property_id];
    ...
    function riskfreeinvest(uint property_id, uint rf_amount) public payable
    {
        ...
        uint returnForRFI = tryMul(2,rf_amount);//0.2% fixed returns
        uint payToRiskFreeInvestor = tryDiv(returnForRFI,1000);
        prop.riskFreeInvestorPayment.push(payToRiskFreeInvestor);
        allproperties[property_id]=prop;
    }

    function trade(uint property_id) public payable
    {
    ...
    function payEmi(uint property_id, uint penaltyAmount)public payable
    {
        ...
        if(penaltyAmount==0)
        require(msg.value==prop.emi_to_be_paid);
        else
        require(msg.value==penaltyAmount);
        ...
        address[] storage allriskfreeinvestors = prop.risk_free_investors;
        uint count_of_rf_investors = allriskfreeinvestors.length;
        uint totalRiskFreePayment=0;
        for(uint i=0;i<count_of_rf_investors;i++)
        {
            payable(allriskfreeinvestors[i]).transfer(prop.riskFreeInvestorPayment[i]);
        ...
        nftContract.transferFromTCToMortgager(tc,prop.mortgager,prop.nftTokenId);
         
         // transfer investment to risk free investors
        address[] storage allriskfreeinvestors1 = prop.risk_free_investors;
        uint count_of_rf_investors1 = allriskfreeinvestors1.length;
        for(uint i=0;i<count_of_rf_investors1;i++)
        {
            payable(allriskfreeinvestors1[i]).transfer(prop.riskFreeInvestorInvestment[i]);
        }
    ...
    function snatchNFT(uint property_id)public payable
    { 
        House_property storage prop = allproperties[property_id];
        require(msg.sender==tc);
        require(msg.value==prop.price);
        uint no_of_months_emi_paid = prop.totalInterestMonths - prop.interestMonthsLeft;
        uint totalEMIPaid = tryMul(no_of_months_emi_paid,prop.emi_to_be_paid);
        uint totalEMItoBePaid = tryMul(prop.totalInterestMonths,prop.emi_to_be_paid);
        // If mortgager defaults then, calculate his liabilities
        uint actualLiability = totalEMItoBePaid - totalEMIPaid;
        uint buyerExitAmount = prop.price - actualLiability;
        ...
        for(uint i=0;i<count_of_rf_investors;i++)
        {
            payable(allriskfreeinvestors[i]).transfer(prop.riskFreeInvestorInvestment[i]);
        }
    ...
    function getHouseDetails(uint property_id) public view returns(House_property memory)
    {
        return allproperties[property_id];
    }

    function getAddresses(uint property_id) public view returns(address[] memory)
    {
        return allproperties[property_id].risk_free_investors;
    }

    function getRiskFreeInvestorInvestment(uint property_id) public view returns(uint[] memory)
    {
        return allproperties[property_id].riskFreeInvestorInvestment;
    }

    function getRiskFreeInvestorPayment(uint property_id) public view returns(uint[] memory)
    {
